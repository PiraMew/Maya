...
#### Tree file
trfn = file.path(gwd, "2_phylo_dating", "4_babette_beast", "no_gdis_concat_annot_lab.nw") #open new tree without outgroup
...
# load the saved result.
runslow = TRUE
resfn = file.path(wd,paste0("MB_DEC.Rdata"))
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
runslow = TRUE
resfn = file.path(wd,paste0("MB_DIVALIKE.Rdata"))
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
runslow = TRUE
resfn = file.path(wd,paste0("MB_BAYAREALIKE.Rdata"))
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
# load the saved result.
runslow = TRUE
resfn = file.path(wd,paste0("MF_DEC.Rdata"))
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
runslow = TRUE
resfn = file.path(wd,paste0("MF_DIVALIKE.Rdata"))
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
runslow = TRUE
resfn = file.path(wd,paste0("MF_BAYAREALIKE.Rdata"))
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
#MB: Baker
res.dec_MB = extract_params_from_BioGeoBEARS_results_object(results_object=resDEC_MB, returnwhat="table", paramsstr_digits=4)
res.dl_MB = extract_params_from_BioGeoBEARS_results_object(results_object=resDIVALIKE_MB, returnwhat="table", paramsstr_digits=4)
res.b_MB = extract_params_from_BioGeoBEARS_results_object(results_object=resBAYAREALIKE_MB, returnwhat="table", paramsstr_digits=4)

#MF: Federman
res.dec_MF = extract_params_from_BioGeoBEARS_results_object(results_object=resDEC_MF, returnwhat="table", paramsstr_digits=4)
res.dl_MF = extract_params_from_BioGeoBEARS_results_object(results_object=resDIVALIKE_MF, returnwhat="table", paramsstr_digits=4)
res.b_MF = extract_params_from_BioGeoBEARS_results_object(results_object=resBAYAREALIKE_MF, returnwhat="table", paramsstr_digits=4)


# put all results into one table
restable=rbind(res.dec_M0,res.dl_M0,res.b_M0,res.dec_MB,res.dl_MB,res.b_MB,res.dec_MF,res.dl_MF,res.b_MF)
row.names(restable) = c("dec_M0", "dl_M0", "b_M0","dec_MB","dl_MB","b_MB","dec_MF","dl_MF","b_MF")
restable
